...
function Shop({ setCart, truncateText }) {
    ...
    //  Видаляємо перший елемент "All", щоб отримати список жанрів без нього  
    ...
    // Завантаження книг (для секції bestsellers)
    useEffect(() => {
    ...
    // Завантаження книг (для секції browse genres)
    useEffect(() => {
                ...
                for (const genre of filteredGenres) {
                    ...
                    for (const item of response.data.docs) {
                        try {
                            ...
                            if (pages && title && author && photo && price >= 50) {
                                booksData.push({
                                    id: item.key,
                                    title,
                                    author,
        ...
        return (
            <div className="pagination">
                {pages.map((page, index) => (
                    <span key={index} className={page === currentPage ? "active_page" : ""} onClick={() => typeof page === "number" && setCurrentPage(page)}>
                        {page}
                    </span>
                ))}
            </div>
    ...
    // Визначення кількості книг на сторінці в залежності від ширини екрану для browse genres
    ...
    // Завантаження книг (для секції offers)
    useEffect(() => {
        ...
        return (
            <div className="pagination">
                {pages.map((page, index) => (
                    <span
                        key={index}
                        className={page === currentOffersPage ? "active_page" : ""}
    ...
    // Витягуємо книги для поточної сторінки для offers
        ...
        <main>
            <section className="section_wrapper">  {/* стилі для section в index.scss*/}
                <div className="intro_box"> {/* стилі для intro_box в index.scss*/}
                    <h2 className="no_margin section_title">What book are you looking for?</h2> {/* стилі для no_margin в index.scss*/}
                ...
                <div className="shapes">
                    {books.map((book, index) => (
                        <div className="arched_conteiner" key={index}>
            ...
            <section>
                ...
                <div className="catalog">
                    <ul className="genres_list">
                        ...
                        ))} {/* стилі для active вкладені в стилі genres_list*/}
                    </ul>
